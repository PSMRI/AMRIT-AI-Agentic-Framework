...
metadata:
  ...
  supported_inputs:
    - Android scope assigned by amrit-implement-jira-ticket
...
This skill is normally invoked by `amrit-implement-jira-ticket`, which owns persona routing and coordination. It can also be invoked directly for an Android-only change, and does not require the orchestrator to be installed.

```text
/amrit-implement-android-change AMRIT-1234
```
...
- schema in `AMRIT-DB` — `amrit-implement-database-change`;
