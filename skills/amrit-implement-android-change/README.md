...
`amrit-implement-android-change` is the **Android / Kotlin Developer** specialist for Stage 05 — In Development. It implements the mobile portion of one approved AMRIT Jira ticket in a Kotlin Android repository such as `FLW-Mobile-App` or `HWC-Mobile-App`.
...
`amrit-implement-jira-ticket` routes to this skill only when the Android application is actually in scope.
...
Invoke `/amrit-implement-android-change` from the repository root using a supported coding agent, or let `/amrit-implement-jira-ticket` route to it. For a packaged installation, download `amrit-implement-android-change.zip` from the latest release. See the
