...
metadata:
  ...
  supported_inputs:
    - Backend scope assigned by amrit-implement-jira-ticket
  ...
  parent_skill: amrit-implement-jira-ticket
  next_skill: amrit-write-unit-tests
---
...
This skill is normally invoked by `amrit-implement-jira-ticket`, which owns persona routing, cross-layer coordination, and the final report. It can also be invoked directly for a backend-only change. It does not require the orchestrator to be installed.

```text
/amrit-implement-backend-change AMRIT-1234
```
...
- schema DDL, migrations, indexes, and constraints — `amrit-implement-database-change` in `AMRIT-DB`;
- web UI — `amrit-implement-frontend-change`;
- Android — `amrit-implement-android-change`;
- unit tests — `amrit-write-unit-tests`, although this skill must leave the code testable and state what needs coverage;
- branches, commits, pushes, and Pull Requests — `amrit-create-development-pr`.
...
A true schema change belongs to `amrit-implement-database-change` and lives in `AMRIT-DB`. This skill implements only the application-side entities, mappings, repositories, queries, DTOs, and validation needed to use that schema, and keeps them compatible with it. If the required schema change does not exist yet, do not invent it and do not create a local substitute migration: report the dependency and stop the dependent work.

### 6. Report

Produce the completion output below. Note what needs unit-test coverage; `amrit-write-unit-tests` writes the tests.
...
- <behaviour that amrit-write-unit-tests must cover>
