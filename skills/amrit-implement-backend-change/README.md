...
`amrit-implement-backend-change` is the **Backend Developer** specialist for Stage 05 — In Development. It implements the server-side portion of one approved AMRIT Jira ticket in a Spring Boot API repository.
...
`amrit-implement-jira-ticket` is the normal Stage 05 entry point. It reads the ticket, researches the knowledge sources, inspects the code, classifies the impacted personas, and invokes this skill only when the ticket actually changes the backend.
...
Any real schema change belongs to `amrit-implement-database-change` and lives in `AMRIT-DB`. This skill implements only the application-side entities, mappings, queries, DTOs, and validation that use that schema, and it classifies the persistence impact explicitly as no database change, application model or query change only, or database schema change.
...
Invoke `/amrit-implement-backend-change` from the repository root using a supported coding agent, or let `/amrit-implement-jira-ticket` route to it. For a packaged installation, download `amrit-implement-backend-change.zip` from the latest release. See the
