...
`amrit-review-implementation-architecture` is the **Technical Architect** specialist for Stage 05 — In Development. It checks that an implementation stays consistent with the approved Stage 03 technical design.
...
`amrit-implement-jira-ticket` selects this persona only for architecturally significant or cross-cutting work: before implementation to fix the constraints, and briefly after implementation when the change spans modules or repositories.
...
- It does not produce a new technical design; that is [`amrit-create-technical-design`](../amrit-create-technical-design/README.md).
...
Invoke `/amrit-review-implementation-architecture` from the repository root using a supported coding agent, or let `/amrit-implement-jira-ticket` route to it. For a packaged installation, download `amrit-review-implementation-architecture.zip` from the latest release. See the
