...
`amrit-perform-root-cause-analysis` investigates AMRIT production defects, support
...
```text
/amrit-perform-root-cause-analysis AMRIT-1234
```
...
This is intentionally different from the `amrit-answer-codebase-questions` research
...
development workflow, typically `amrit-implement-jira-ticket`.
...
```text
/amrit-perform-root-cause-analysis DEMO-4521
```
...
Invoke `/amrit-perform-root-cause-analysis` from the repository root using a
...
For a packaged installation, download `amrit-perform-root-cause-analysis.zip` from
