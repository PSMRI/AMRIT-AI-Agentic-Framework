...
`amrit-draft-test-cases` is the **QA Test Analyst** specialist for Stage 03 — Analysis. It answers one question:
...
| --- | --- | --- |
| `amrit-draft-test-cases` | Stage 03 — Analysis | Functional QA test **specifications** |
...
amrit-test-jira-ticket
    └── Stage 03 — Analysis
        └── amrit-draft-test-cases        ← this skill
```

[`amrit-test-jira-ticket`](../amrit-test-jira-ticket/README.md) selects this skill when the ticket is at Analysis, or when the user explicitly asks for test-case design. The skill is independently installable and independently invocable; when it is not installed, the meta-skill applies the QA Test Analyst contract inline and says so.
...
```text
/amrit-draft-test-cases DEMO-4102
```
...
Invoke `/amrit-draft-test-cases` from the repository root using a supported coding agent, or let `/amrit-test-jira-ticket` route to it. For a packaged installation, download `amrit-draft-test-cases.zip` from the latest release. See the
