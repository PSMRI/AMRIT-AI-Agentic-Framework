...
Designing functional QA test specifications from an agreed requirement. Executable developer-level unit-test code belongs to `amrit-write-unit-tests`. Execution against a deployed build belongs to `amrit-execute-qa-validation`. Keep the three separate; they are different artifacts with different owners.
...
AC states duplicates must be rejected
                       ...
                       → implementation returning 500 is a FAIL for
                         amrit-execute-qa-validation to record
```
...
This field is a recommendation for `amrit-execute-qa-validation`, not a promise that automation exists. Never mark a case automatable because automating it would be convenient.
