...
Acceptance Criterion          AC-3
        ↓
QA Test Case                  TC-07                 ← amrit-draft-test-cases produces this
        ↓
...
`amrit-draft-test-cases` owns the segment from the business requirement to the test case. Everything below the test case is produced later by other skills, and it can only be linked back if the identifiers this skill assigns are stable.
...
`amrit-execute-qa-validation` consumes this specification and records, per test case, the expected result, the actual result, the verdict, and the evidence. It can only do that if:
