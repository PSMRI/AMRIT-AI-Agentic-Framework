...
The shape of one QA test case and of the specification that contains it. Every test case must carry enough structure to be executed later by a human tester or by `amrit-execute-qa-validation` without returning to the author.
...
If a precondition requires work, it is still a precondition — the tester needs it before step 1, and `amrit-execute-qa-validation` needs it to decide whether the case is runnable in the available environment.
