...
`amrit-test-jira-ticket` is the **testing meta-skill** and the entry point for testing work across the AMRIT lifecycle. It takes one Jira ticket, works out where it actually is, establishes what testing evidence already exists, and runs the testing activity that position genuinely calls for.
...
| Stage 06 — Pending QA | Usually none | — | Existing test cases may be reviewed through `amrit-draft-test-cases` |
...
| `amrit-write-unit-tests` | An implementation exists and its source is accessible | Do not route; report that there is nothing to test yet |
| `amrit-execute-qa-validation` | A reachable deployed build | Blocked report with `Executed: 0`; never a PASS |
...
- [`amrit-implement-jira-ticket`](../amrit-implement-jira-ticket/README.md) — **implement this ticket.**
- `amrit-test-jira-ticket` — **perform the appropriate testing activity for this ticket's lifecycle position.**

They do not compete.

## `amrit-write-unit-tests` is reused, never duplicated

`amrit-write-unit-tests` participates in both orchestration paths:
...
One specialist, two doors. Its Stage 05 relationship with `amrit-implement-jira-ticket` — selected whenever production behaviour changed, ahead of `amrit-create-development-pr` — is unchanged by this skill, and no unit-testing logic is copied into the meta-skill.

## Lifecycle exclusions

No skill exists for, and none is wanted for:

- **Stage 06 — Pending QA** — a lifecycle and queue state. Existing test cases may be reviewed through `amrit-draft-test-cases`; there is no `prepare-qa-handoff`.
...
Jira and Confluence are read-only — the only exception being a defect explicitly authorized under `amrit-execute-qa-validation`'s own rules. No branch, commit, push, or Pull Request. No production code is modified in response to a QA failure. No QA approval, code review, CI result, or release approval is ever claimed.
...
```text
/amrit-test-jira-ticket AMRIT-101    Analysis        → amrit-draft-test-cases
/amrit-test-jira-ticket AMRIT-102    In Development  → amrit-write-unit-tests
/amrit-test-jira-ticket AMRIT-103    In QA           → amrit-execute-qa-validation
```
...
Invoke `/amrit-test-jira-ticket` from the repository root using a supported coding agent. For a packaged installation, download `amrit-test-jira-ticket.zip` from the latest release, together with the testing specialist packages you want available. See the
