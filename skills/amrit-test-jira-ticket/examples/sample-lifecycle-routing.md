...
```text
/amrit-test-jira-ticket AMRIT-101
...
- amrit-write-unit-tests — no implementation exists; there is no changed behaviour to
  cover, and unit tests must never be invented for code that does not exist
- amrit-execute-qa-validation — no deployed build exists; QA execution would have
  nothing to test and could only produce a fabricated result

### Specialist invoked

- amrit-draft-test-cases — via the host skill mechanism
```
...
```text
/amrit-test-jira-ticket AMRIT-102
...
Considered and excluded:
- amrit-draft-test-cases — QA test cases already exist and were agreed at Stage 03;
  regenerating them now would risk replacing agreed scope with the
  implementation's behaviour
- amrit-execute-qa-validation — no deployed QA build; the change has not reached QA

### Specialist invoked

- amrit-write-unit-tests — via the host skill mechanism
```

Note what this route did **not** do. The Stage 05 relationship is unchanged: `amrit-implement-jira-ticket` still selects `amrit-write-unit-tests` during implementation, and this run reaches the same specialist by a different door. No unit-testing logic is duplicated in the meta-skill.

If the same ticket had **no** QA test cases, the report would additionally select `amrit-draft-test-cases` and note that a Stage 04 Definition of Ready artifact was missing.

## Example 3 — QA

```text
/amrit-test-jira-ticket AMRIT-103
...
Considered and excluded:
- amrit-draft-test-cases — an agreed specification already exists; regenerating it
  from the implementation would replace agreed QA scope with whatever the code
  does
- amrit-write-unit-tests — the implementation is complete and its unit tests ran
  green in CI; a green unit suite is not QA validation and does not substitute
  for it

### Specialist invoked

- amrit-execute-qa-validation — via the host skill mechanism
```
...
/amrit-test-jira-ticket AMRIT-101   →  amrit-draft-test-cases
                              →  amrit-write-unit-tests        ← no implementation
                              →  amrit-execute-qa-validation   ← no build
```
...
/amrit-test-jira-ticket AMRIT-103   →  amrit-draft-test-cases regenerated from the
