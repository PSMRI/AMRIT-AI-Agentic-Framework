...
Both continue Example 3 from [sample-lifecycle-routing.md](sample-lifecycle-routing.md): a ticket routed to `amrit-execute-qa-validation` at Stage 07.
...
- AC-3 is not satisfied. The work returns to the implementation flow —
  amrit-implement-jira-ticket and its specialists — through the defect and rework
  path.
