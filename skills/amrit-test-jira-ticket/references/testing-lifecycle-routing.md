...
```text
Stage 03 — Analysis                  amrit-draft-test-cases
Stage 04 — Ready for Development     no testing specialist required
Stage 05 — In Development            amrit-write-unit-tests
Stage 06 — Pending QA                no dedicated skill
Stage 07 — In QA                     amrit-execute-qa-validation
...
| Existing QA test cases | Confluence, the configured test-management source, or a prior `amrit-draft-test-cases` output | Decides design versus execution, and whether execution has agreed scope |
...
| --- | --- | --- | --- | --- |
| 03 — Analysis | absent | absent | — | `amrit-draft-test-cases` |
| 03 — Analysis | absent | present | — | `amrit-draft-test-cases` to review and extend, or report the specification as already complete |
| 04 — Ready for Development | absent | present | — | No activity; report the ticket as test-ready |
| 04 — Ready for Development | absent | absent | — | `amrit-draft-test-cases` — a Definition of Ready artifact is missing |
| 05 — In Development | present | — | — | `amrit-write-unit-tests`; report a missing QA specification as a gap |
| 05 — In Development | absent | — | — | No unit testing is possible; report that there is nothing to test yet |
| 06 — Pending QA | present | present | — | No activity; the specification may be reviewed through `amrit-draft-test-cases` on request |
| 06 — Pending QA | present | absent | — | `amrit-draft-test-cases` — QA has no agreed scope to execute |
| 07 — In QA | present | present | yes | `amrit-execute-qa-validation` |
| 07 — In QA | present | present | no | `amrit-execute-qa-validation` producing its blocked report, or report the gap directly. Never a PASS |
| 07 — In QA | present | absent | yes | `amrit-execute-qa-validation` criteria-driven, and recommend `amrit-draft-test-cases` |
...
| --- | --- | --- |
| `amrit-draft-test-cases` | Acceptance criteria or an approved requirement set | Stop. Report that the requirement is missing and name what is needed. Never invent a requirement to have something to test. |
| `amrit-write-unit-tests` | An implementation exists and its source is accessible | Do not route. Report that there is nothing to test yet. Never invent tests for code that does not exist, and never write tests against a documented design. |
| `amrit-execute-qa-validation` | A reachable deployed build | Route so the specialist produces its blocked report with `Executed: 0`, or report the gap directly. Never a PASS, and never a documentation review presented as validation. |
...
- amrit-draft-test-cases — the ticket has acceptance criteria and no agreed QA
  test cases
...
| Status says In Development, no code exists | Report it; offer `amrit-draft-test-cases` if QA test cases are missing |
| Status says Analysis, an implementation already exists | Still route to `amrit-draft-test-cases`, and design from the requirement — never from the implementation. Note the divergence |
...
```text
Selected:
- amrit-write-unit-tests    — implementation exists in <repository>
- amrit-draft-test-cases    — no QA test cases exist; a Stage 04 Definition of
                        Ready artifact is missing

Excluded:
- amrit-execute-qa-validation — no deployed build; the change is not released to QA
```
...
| Stage 06 — Pending QA | A lifecycle and queue state | Existing QA test cases may be reviewed or refined through `amrit-draft-test-cases` on request |
| Stage 08 — QA Approved | A human accountability gate | `amrit-execute-qa-validation` supplies evidence; the human decides |
