...
Jira is read-only throughout. The only permitted write anywhere in this architecture is a defect explicitly authorized under `amrit-execute-qa-validation`'s own rules.
...
| Existing QA test cases | Confluence, the configured test-management source, or a prior `amrit-draft-test-cases` output |
...
```text
amrit-draft-test-cases          before   amrit-execute-qa-validation
implementation exists     before   amrit-write-unit-tests
```
...
Every skill in this framework is independently installable, so an environment may have `amrit-test-jira-ticket` without its specialists.
...
- It never writes QA test cases when `amrit-draft-test-cases` is available.
- It never writes unit-test code when `amrit-write-unit-tests` is available.
- It never executes QA validation when `amrit-execute-qa-validation` is available.
