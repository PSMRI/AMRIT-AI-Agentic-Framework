...
| Segment | Owner | Stage |
| --- | --- | --- |
| Business Requirement → FRD → Jira Story → Acceptance Criterion | Upstream skills — `amrit-create-brd`, `amrit-create-product-backlog` | 01, 02 |
| Acceptance Criterion → QA Test Case | `amrit-draft-test-cases` | 03 |
| Acceptance Criterion → Implementation | `amrit-implement-jira-ticket` and its specialists | 05 |
| Implementation → Unit Tests | `amrit-write-unit-tests` | 05 |
| QA Test Case → QA Execution → Evidence → PASS / Defect | `amrit-execute-qa-validation` | 07 |
| Recording which segment this run produced, and which are missing | `amrit-test-jira-ticket` | Cross-stage |

`amrit-write-unit-tests` sits in this chain once, under both orchestration paths. `amrit-implement-jira-ticket` selects it during implementation; `amrit-test-jira-ticket` selects it when development-level testing is explicitly appropriate. Same specialist, same segment, no duplication.
...
AC-4               → no test case         → gap: amrit-draft-test-cases required
```
...
| Break | Report |
| --- | --- |
| An acceptance criterion has no test case | Gap — `amrit-draft-test-cases` required |
...
| Changed behaviour has no unit test | Gap — `amrit-write-unit-tests` required |
...
| --- | --- | --- |
| QA test specification | `amrit-draft-test-cases` | Test code; execution results |
| Unit-test code and its results | `amrit-write-unit-tests` | A QA specification; QA validation |
| QA execution results and evidence | `amrit-execute-qa-validation` | A specification; a unit-test run |
