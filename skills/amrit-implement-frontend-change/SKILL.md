...
metadata:
  ...
  supported_inputs:
    - Frontend scope assigned by amrit-implement-jira-ticket
...
This skill is normally invoked by `amrit-implement-jira-ticket`, which owns persona routing and coordination. It can also be invoked directly for a frontend-only change, and does not require the orchestrator to be installed.

```text
/amrit-implement-frontend-change AMRIT-1234
```
...
- server-side behaviour and API contracts — `amrit-implement-backend-change`;
- Android — `amrit-implement-android-change`;
- schema — `amrit-implement-database-change`;
- unit tests — `amrit-write-unit-tests`;
- approved UX conformance judgement — `amrit-validate-ux-implementation`;
- Git and Pull Request work — `amrit-create-development-pr`.
