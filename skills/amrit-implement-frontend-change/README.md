...
`amrit-implement-frontend-change` is the **Frontend Developer** specialist for Stage 05 — In Development. It implements the web UI portion of one approved AMRIT Jira ticket in an Angular UI repository.
...
`amrit-implement-jira-ticket` is the normal Stage 05 entry point. It routes to this skill only when the ticket actually changes the web UI.

```text
amrit-implement-jira-ticket
    ├── amrit-implement-backend-change     (establishes the API contract)
    └── amrit-implement-frontend-change    (consumes it)
...
Where approved wireframes and design-system rules exist, the implementation follows them. New product design is not invented in Stage 05; gaps are reported. `amrit-validate-ux-implementation` performs the separate conformance check.
...
Invoke `/amrit-implement-frontend-change` from the repository root using a supported coding agent, or let `/amrit-implement-jira-ticket` route to it. For a packaged installation, download `amrit-implement-frontend-change.zip` from the latest release. See the
