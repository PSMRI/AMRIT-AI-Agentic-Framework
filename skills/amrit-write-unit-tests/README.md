...
`amrit-write-unit-tests` is the **SDET / developer-testing** specialist for Stage 05 — In Development. It adds or updates code-level unit tests for an implemented AMRIT change and runs them.
...
`amrit-implement-jira-ticket` invokes this skill whenever production behaviour changed — effectively on every implementation route.
...
amrit-implement-jira-ticket
    ...
    └── amrit-write-unit-tests
        └── verification → amrit-create-development-pr
```
...
| --- | --- | --- |
| [`amrit-draft-test-cases`](../amrit-draft-test-cases/README.md) | Stage 03 — Analysis | Functional QA test **specifications** |
| `amrit-write-unit-tests` | Stage 05 — In Development | Executable **unit-test code** with real results |
| [`amrit-execute-qa-validation`](../amrit-execute-qa-validation/README.md) | Stage 07 — In QA | QA **execution results and evidence** |
...
Invoke `/amrit-write-unit-tests` from the repository root using a supported coding agent, or let `/amrit-implement-jira-ticket` route to it. For a packaged installation, download `amrit-write-unit-tests.zip` from the latest release. See the
