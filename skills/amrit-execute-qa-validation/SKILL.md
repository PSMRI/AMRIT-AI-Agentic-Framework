...
metadata:
  ...
  upstream_producer: amrit-draft-test-cases
...
```text
/amrit-execute-qa-validation AMRIT-1234
```

This skill is normally invoked by `amrit-test-jira-ticket` when the ticket is at Stage 07. It can also be invoked directly, and does not require the meta-skill to be installed.

## Scope boundary

This skill **executes** QA validation. It is deliberately separate from:

- `amrit-draft-test-cases` — designing the QA test specification, Stage 03;
- `amrit-write-unit-tests` — writing and running developer-level unit tests, Stage 05.
...
Find the test specification produced at Stage 03 by `amrit-draft-test-cases`, or the equivalent agreed test cases held in Confluence or the configured test-management source.
...
3. recommend `amrit-draft-test-cases` so the ticket has a reviewed specification for future runs.
...
**Never fix production code from this skill.** A QA failure returns the work to the implementation flow — `amrit-implement-jira-ticket` and its specialists — not to a QA-side patch. Changing the code to make a test pass destroys the independence that makes QA validation meaningful.
