...
`amrit-execute-qa-validation` is the **QA Tester / QA Automation Engineer** specialist for Stage 07 — In QA. It answers one question:
...
| [`amrit-write-unit-tests`](../amrit-write-unit-tests/README.md) | Stage 05 — In Development | Executable developer-level **unit-test code** |
| `amrit-execute-qa-validation` | Stage 07 — In QA | QA **execution results and evidence** |
...
amrit-test-jira-ticket
    └── Stage 07 — In QA
        └── amrit-execute-qa-validation        ← this skill
```

[`amrit-test-jira-ticket`](../amrit-test-jira-ticket/README.md) selects this skill when the ticket is at In QA and a build exists. The skill is independently installable and independently invocable; when it is not installed, the meta-skill applies the QA execution contract inline and says so.
...
```text
/amrit-execute-qa-validation DEMO-4102
```
...
Invoke `/amrit-execute-qa-validation` from the repository root using a supported coding agent, or let `/amrit-test-jira-ticket` route to it. For a packaged installation, download `amrit-execute-qa-validation.zip` from the latest release. See the
