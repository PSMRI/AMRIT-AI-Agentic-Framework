...
Executing or coordinating QA validation of one ticket against a deployed QA build at Stage 07 — In QA. Designing the test cases belongs to `amrit-draft-test-cases`. Developer-level unit testing belongs to `amrit-write-unit-tests`.
...
Take the specification produced at Stage 03 by `amrit-draft-test-cases`, or the equivalent agreed test cases in Confluence or the configured test-management source.
...
When no agreed test cases exist, state that plainly, validate against the acceptance criteria directly, label the run criteria-driven, and recommend `amrit-draft-test-cases` for a reviewed specification. Never invent a specification and present its results as agreed QA scope.
