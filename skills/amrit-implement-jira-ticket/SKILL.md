...
metadata:
  ...
  next_skill: amrit-create-development-pr
---
...
This skill changes source files, directly or through the specialists it invokes. It is deliberately different from the read-only `amrit-create-technical-design` and `amrit-answer-codebase-questions` skills.
...
Use the framework's knowledge sources in the order established by `amrit-answer-codebase-questions`: DeepWiki first for repository architecture, module responsibilities, major flows, abstractions, integration boundaries, and existing implementation patterns; Confluence for intended architecture and design rationale; Graphify only as the final fallback for unresolved cross-repository relationships.
...
Code-level unit tests are mandatory for every changed behaviour. `amrit-write-unit-tests` owns them. This is developer testing, not Stage 07 `amrit-execute-qa-validation`, and it is distinct from `amrit-draft-test-cases`.

`amrit-write-unit-tests` also participates in the testing meta-skill `amrit-test-jira-ticket`. That does not change its selection here: this orchestrator continues to select it whenever production behaviour changed, ahead of `amrit-create-development-pr`.
...
Finish by naming `amrit-create-development-pr` as the next skill. This skill never performs Git or Pull Request work.
...
| --- | --- | --- |
| Technical Architect | `amrit-review-implementation-architecture` | cross-cutting or cross-repository change, a new component or contract, a changed integration or module boundary, security- or performance-material change, or a possible deviation from the approved Stage 03 design |
| DBA / Database Engineer | `amrit-implement-database-change` | a schema object must be created, altered, or removed, or a migration, index, constraint, or data-compatibility concern exists |
| Backend Developer | `amrit-implement-backend-change` | server-side services, APIs, controllers, domain logic, integrations, validation, persistence integration, error handling, or backend configuration change |
| Frontend Developer | `amrit-implement-frontend-change` | web UI components, state management, API integration, forms, client validation, accessibility, or frontend error handling change |
| Android / Kotlin Developer | `amrit-implement-android-change` | the Android applications change, including Kotlin implementation, mobile flows, API integration, offline behaviour, or device and platform constraints |
| UX / UI Specialist | `amrit-validate-ux-implementation` | a user-visible change exists and approved wireframes, workflows, or design-system rules must be honoured |
| SDET / developer testing | `amrit-write-unit-tests` | production code changed — effectively always |
...
- `amrit-write-unit-tests` runs whenever production behaviour changed.
- `amrit-review-implementation-architecture` does not re-open an approved Stage 03 design. During Stage 05 it checks conformance.
- `amrit-validate-ux-implementation` validates the implementation against approved UX. It does not invent product design when approved UX already exists.
...
Full stack              amrit-implement-database-change? → amrit-implement-backend-change → amrit-implement-frontend-change
                        → amrit-validate-ux-implementation → amrit-write-unit-tests
Android                 amrit-implement-android-change → amrit-write-unit-tests
Architecturally         amrit-review-implementation-architecture → amrit-implement-database-change
significant             → amrit-implement-backend-change → amrit-implement-frontend-change
                        → amrit-validate-ux-implementation → amrit-write-unit-tests
```
...
- the report names `amrit-create-development-pr` and ends with the correct completion line.
