...
Accept the ticket key from the invocation, for example `/amrit-implement-jira-ticket AMRIT-1234` or `Implement AMRIT-1234`. If no key is supplied, ask for one before researching.
...
When one ticket legitimately spans repositories, keep the changes traceable per repository. They are separate Git repositories and may need separate Pull Requests downstream; that is `amrit-create-development-pr`'s decision, not this skill's.
...
Whenever production behaviour changed, `amrit-write-unit-tests` runs and covers the changed behaviour: happy path, validation, error behaviour, boundary cases, and regression risk, with dependencies mocked as the repository already does.

This is developer, code-level testing. It is not `amrit-execute-qa-validation` at Stage 07 and it is not `amrit-draft-test-cases` at Stage 03.
...
Name `amrit-create-development-pr` as the next skill and finish with exactly one of:
