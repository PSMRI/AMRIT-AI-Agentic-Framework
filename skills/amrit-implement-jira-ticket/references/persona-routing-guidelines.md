...
### Technical Architect — `amrit-review-implementation-architecture`
...
### DBA / Database Engineer — `amrit-implement-database-change`
...
### Backend Developer — `amrit-implement-backend-change`
...
### Frontend Developer — `amrit-implement-frontend-change`
...
### Android / Kotlin Developer — `amrit-implement-android-change`
...
### UX / UI Specialist — `amrit-validate-ux-implementation`
...
### SDET / developer testing — `amrit-write-unit-tests`
...
Distinct from `amrit-draft-test-cases` at Stage 03 and from `amrit-execute-qa-validation` at Stage 07.

This specialist is shared with the testing meta-skill `amrit-test-jira-ticket`, which routes to the same skill when development-level testing is explicitly appropriate. That does not change its selection here.
...
```text
amrit-implement-jira-ticket
    ├── amrit-implement-database-change      (first — establishes the schema contract)
    ├── amrit-implement-backend-change       (consumes the schema)
    └── amrit-write-unit-tests
```
...
```text
amrit-implement-jira-ticket
    ├── amrit-implement-backend-change       (establishes the API contract)
    ├── amrit-implement-frontend-change      (consumes the API contract)
    └── amrit-write-unit-tests
```
...
```text
amrit-implement-jira-ticket
    ├── amrit-review-implementation-architecture   (before implementation)
...
Each skill in this framework is independently installable, so an environment may have `amrit-implement-jira-ticket` without its specialists.
