...
```text
amrit-review-implementation-architecture     (before implementation, when selected)
        ↓
amrit-implement-database-change              (schema first — it constrains everything above it)
        ↓
amrit-implement-backend-change               (models, then persistence, then API)
        ↓
amrit-implement-frontend-change / amrit-implement-android-change   (consume the API contract)
        ↓
amrit-validate-ux-implementation             (after the user-visible change exists)
        ↓
amrit-write-unit-tests                       (covers everything that changed)
        ↓
verification
        ↓
...
- This skill edits repositories. It never commits in any of them; downstream `amrit-create-development-pr` decides how the change set becomes one or more Pull Requests.
