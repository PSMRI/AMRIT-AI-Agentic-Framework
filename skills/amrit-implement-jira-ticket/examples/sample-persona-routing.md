...
```text
amrit-implement-jira-ticket
    ├── amrit-implement-database-change      (first — publishes the schema contract)
    ├── amrit-implement-backend-change       (consumes the schema contract)
    └── amrit-write-unit-tests
```
...
amrit-implement-jira-ticket
    ...
    ├── amrit-validate-ux-implementation     (approved screen layout exists)
    └── amrit-write-unit-tests
```
