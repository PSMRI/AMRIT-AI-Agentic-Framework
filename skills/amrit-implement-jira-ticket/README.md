...
**This skill changes source files**, directly or through the specialists it invokes. That makes it intentionally different from the read-only `amrit-create-technical-design` and `amrit-answer-codebase-questions` skills.
...
Users do not select personas by hand. There is no need to invoke `/amrit-implement-backend-change`, `/amrit-implement-database-change`, or `/amrit-write-unit-tests` separately for ordinary ticket implementation: this skill routes the ticket. The specialists remain independently invocable for focused work.
...
| --- | --- | --- |
| Technical Architect | [`amrit-review-implementation-architecture`](../amrit-review-implementation-architecture/README.md) | cross-cutting or cross-repository change, new component or contract, changed ownership or integration boundary, security- or performance-material change, suspected design deviation |
| DBA / Database Engineer | [`amrit-implement-database-change`](../amrit-implement-database-change/README.md) | a schema object, migration, index, constraint, or data-compatibility concern exists |
| Backend Developer | [`amrit-implement-backend-change`](../amrit-implement-backend-change/README.md) | services, APIs, controllers, domain logic, integrations, validation, persistence integration, error handling, or backend configuration change |
| Frontend Developer | [`amrit-implement-frontend-change`](../amrit-implement-frontend-change/README.md) | web UI, components, state, API integration, forms, client validation, accessibility, or frontend error handling change |
| Android / Kotlin Developer | [`amrit-implement-android-change`](../amrit-implement-android-change/README.md) | the Android applications change, including offline behaviour and platform constraints |
| UX / UI Specialist | [`amrit-validate-ux-implementation`](../amrit-validate-ux-implementation/README.md) | a user-visible change exists and approved UX or design-system rules apply |
| SDET / developer testing | [`amrit-write-unit-tests`](../amrit-write-unit-tests/README.md) | production behaviour changed — effectively always |
...
This matches the knowledge-source philosophy of [`amrit-answer-codebase-questions`](../amrit-answer-codebase-questions/README.md).
...
Any actual schema change is implemented in `AMRIT-DB` by `amrit-implement-database-change`, never in an application repository for convenience. If `AMRIT-DB` is unavailable, no local substitute migration is created; the required change is stated, the dependent work stops, and the implementation is reported incomplete.

## Relationship to `amrit-test-jira-ticket`
...
- `amrit-implement-jira-ticket` — **implement this ticket.**
- [`amrit-test-jira-ticket`](../amrit-test-jira-ticket/README.md) — **perform the appropriate testing activity for this ticket's lifecycle position.**

`amrit-write-unit-tests` belongs to both paths. Its Stage 05 relationship here is unchanged: this orchestrator selects it whenever production behaviour changed, ahead of `amrit-create-development-pr`. The testing meta-skill routes to the same specialist when development-level testing is explicitly appropriate; nothing is duplicated.

## Test and verification behaviour

Unit tests are mandatory for changed behaviour and are owned by `amrit-write-unit-tests` — developer, code-level testing, kept conceptually separate from `amrit-draft-test-cases` at Stage 03 and `amrit-execute-qa-validation` at Stage 07.
...
Branch creation, Git operations, PR title and description, PR creation, and review preparation belong to the downstream [`amrit-create-development-pr`](../amrit-create-development-pr/README.md) skill. That skill is independently installable; this one completes without it.
...
```text
/amrit-implement-jira-ticket DEMO-5140
```
...
Invoke `/amrit-implement-jira-ticket` from the repository root using a supported coding agent. Configure local MCP credentials only where the selected client requires them; never commit real tokens.

For a packaged installation, download `amrit-implement-jira-ticket.zip` from the latest GitHub Release, together with the specialist packages you want available. See the
