...
`amrit-validate-ux-implementation` is the **UX / UI specialist** for Stage 05 — In Development. It validates that an implemented AMRIT user interface matches the approved UX.
...
`amrit-implement-jira-ticket` selects this persona only when a user-visible change exists, and runs it after the change has been implemented.

```text
amrit-implement-jira-ticket
    ├── amrit-implement-frontend-change / amrit-implement-android-change
    ├── amrit-validate-ux-implementation      (after the user-visible change exists)
...
Each gap names the owning skill: `amrit-implement-frontend-change`, `amrit-implement-android-change`, or `amrit-implement-backend-change` for backend-driven content. UX review and approval remain human responsibilities and are reported as outstanding.
...
Invoke `/amrit-validate-ux-implementation` from the repository root using a supported coding agent, or let `/amrit-implement-jira-ticket` route to it. For a packaged installation, download `amrit-validate-ux-implementation.zip` from the latest release. See the
