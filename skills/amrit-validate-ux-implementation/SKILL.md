...
metadata:
  ...
  supported_inputs:
    - User-visible change assigned by amrit-implement-jira-ticket
...
It is normally invoked by `amrit-implement-jira-ticket` after a user-visible change exists. It can also be invoked directly against an implemented screen, and does not require the orchestrator to be installed.

```text
/amrit-validate-ux-implementation AMRIT-1234
```
...
- It does not implement or restyle anything. Corrections go back to `amrit-implement-frontend-change` or `amrit-implement-android-change`.
