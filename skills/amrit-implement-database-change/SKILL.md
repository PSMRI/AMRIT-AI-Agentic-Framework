...
metadata:
  ...
  supported_inputs:
    - Database scope assigned by amrit-implement-jira-ticket
  ...
  next_skill: amrit-implement-backend-change
---
...
This skill is normally invoked by `amrit-implement-jira-ticket` before the backend work, because the schema constrains everything above it. It can also be invoked directly, and does not require the orchestrator to be installed.

```text
/amrit-implement-database-change AMRIT-1234
```
...
- application entities, repositories, queries, and DTOs — `amrit-implement-backend-change`;
