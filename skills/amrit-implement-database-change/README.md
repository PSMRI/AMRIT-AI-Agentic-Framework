...
`amrit-implement-database-change` is the **DBA / Database Engineer** specialist for Stage 05 — In Development. It implements the schema portion of one approved AMRIT Jira ticket in the `AMRIT-DB` repository.
...
`amrit-implement-jira-ticket` routes to this skill only when a real schema change exists, and runs it **before** the backend work because the schema constrains everything above it.

```text
amrit-implement-jira-ticket
    ├── amrit-implement-database-change    (schema contract)
    └── amrit-implement-backend-change     (consumes it)
...
Invoke `/amrit-implement-database-change` from the repository root using a supported coding agent, or let `/amrit-implement-jira-ticket` route to it. For a packaged installation, download `amrit-implement-database-change.zip` from the latest release. See the
