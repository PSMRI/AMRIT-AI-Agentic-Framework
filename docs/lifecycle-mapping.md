...
| --- | --- | --- |
| Stage 01 | BRD | [`amrit-create-brd`](../skills/amrit-create-brd/README.md) |
| Stage 02 | Open | [`amrit-create-product-backlog`](../skills/amrit-create-product-backlog/README.md) |
| Stage 03 | Analysis | [`amrit-create-technical-design`](../skills/amrit-create-technical-design/README.md) and [`amrit-draft-test-cases`](../skills/amrit-draft-test-cases/README.md) |
| Stage 04 | Ready for Development | **No skill yet — documented gap.** No testing specialist required |
| Stage 05 | In Development | [`amrit-implement-jira-ticket`](../skills/amrit-implement-jira-ticket/README.md) with its seven conditionally selected specialists, then [`amrit-create-development-pr`](../skills/amrit-create-development-pr/README.md) |
| Stage 06 | Pending QA | **No dedicated skill, deliberately.** Existing QA test cases may be reviewed through `amrit-draft-test-cases` |
| Stage 07 | In QA | [`amrit-execute-qa-validation`](../skills/amrit-execute-qa-validation/README.md) |
...
| Stage 12 | Production Release | [`amrit-prepare-release-notes`](../skills/amrit-prepare-release-notes/README.md) documents the release; [`amrit-create-brd`](../skills/amrit-create-brd/README.md) also serves the Stage 12 feedback loop back into Business & Product |
...
The skills are independently installable. They align to consecutive lifecycle stages, with two meta-skills — one engineering, one testing — their conditionally selected specialists, and one Pull Request skill; `amrit-answer-codebase-questions` is available across the lifecycle. A downstream skill may consume an approved upstream output without requiring the upstream skill itself to be installed.

| Stage | Primary role | Skill | Review status |
| --- | --- | --- | --- |
| Stage 01/12 — Business & Product | Business Systems Analyst | [`amrit-create-brd`](../skills/amrit-create-brd/README.md) | Draft — Pending Human Review |
| Stage 02 — Product Backlog Creation | Product Manager | [`amrit-create-product-backlog`](../skills/amrit-create-product-backlog/README.md) | Draft - Pending Product Manager Review |
| Stage 03 — Engineering Analysis | Technical Architect / Senior Developer | [`amrit-create-technical-design`](../skills/amrit-create-technical-design/README.md) | Ready for Architect Review |
| Stage 03 — Analysis | QA Tester / Test Analyst | [`amrit-draft-test-cases`](../skills/amrit-draft-test-cases/README.md) | QA test specification drafted — Pending QA review |
...
| Stage 05 — In Development | Developer / Senior Developer (orchestration) | [`amrit-implement-jira-ticket`](../skills/amrit-implement-jira-ticket/README.md) | Ready for PR preparation |
| Stage 05 — In Development | Technical Architect | [`amrit-review-implementation-architecture`](../skills/amrit-review-implementation-architecture/README.md) | Conformance assessed — Architect review outstanding |
| Stage 05 — In Development | DBA / Database Engineer | [`amrit-implement-database-change`](../skills/amrit-implement-database-change/README.md) | Migration implemented — DBA review outstanding |
| Stage 05 — In Development | Backend Developer | [`amrit-implement-backend-change`](../skills/amrit-implement-backend-change/README.md) | Backend implemented |
| Stage 05 — In Development | Frontend Developer | [`amrit-implement-frontend-change`](../skills/amrit-implement-frontend-change/README.md) | Frontend implemented |
| Stage 05 — In Development | Android / Kotlin Developer | [`amrit-implement-android-change`](../skills/amrit-implement-android-change/README.md) | Android implemented |
| Stage 05 — In Development | UX / UI Specialist | [`amrit-validate-ux-implementation`](../skills/amrit-validate-ux-implementation/README.md) | UX conformance assessed — UX review outstanding |
| Stage 05 — In Development | SDET / Developer testing | [`amrit-write-unit-tests`](../skills/amrit-write-unit-tests/README.md) | Unit tests executed |
| Stage 05 — In Development | Developer / Senior Developer | [`amrit-create-development-pr`](../skills/amrit-create-development-pr/README.md) | Awaiting code review |
| Stage 07 — In QA | QA Tester / QA Automation Engineer | [`amrit-execute-qa-validation`](../skills/amrit-execute-qa-validation/README.md) | Execution evidence produced — QA approval outstanding |
| Cross-stage — Testing orchestration | Testing orchestrator | [`amrit-test-jira-ticket`](../skills/amrit-test-jira-ticket/README.md) | Testing activity performed for the ticket's lifecycle position |
| Stage 12 — Release Documentation | Release Manager / Product Owner / Technical Lead | [`amrit-prepare-release-notes`](../skills/amrit-prepare-release-notes/README.md) | Draft — Pending Human Confirmation |
| Cross-lifecycle — Support & Quality | Senior Developer / Technical Lead / Support Engineer | [`amrit-perform-root-cause-analysis`](../skills/amrit-perform-root-cause-analysis/README.md) | Evidence-backed RCA with CAPA recommendations, pending human confirmation |
| Cross-lifecycle — Codebase knowledge | Software Engineer | [`amrit-answer-codebase-questions`](../skills/amrit-answer-codebase-questions/README.md) | Evidence-backed codebase answer |
...
`amrit-implement-jira-ticket` answers *implement this ticket*. `amrit-test-jira-ticket` answers *perform the appropriate testing activity for this ticket's lifecycle position*. They do not compete.

`amrit-write-unit-tests` participates in both paths as one specialist, never duplicated:

```text
amrit-implement-jira-ticket → amrit-write-unit-tests     (primary, Stage 05, unchanged)
amrit-test-jira-ticket      → amrit-write-unit-tests     (when development-level testing is
...
```text
                    ...
                    Stage 03 → amrit-draft-test-cases
                    Stage 05 → amrit-write-unit-tests when applicable
                    Stage 07 → amrit-execute-qa-validation
```

| Skill | Stage | Persona | Question answered | Artifact |
| --- | --- | --- | --- | --- |
| [`amrit-draft-test-cases`](../skills/amrit-draft-test-cases/README.md) | Stage 03 — Analysis | QA Tester / Test Analyst | What must QA test to prove this requirement works? | Functional QA test **specifications** |
| [`amrit-write-unit-tests`](../skills/amrit-write-unit-tests/README.md) | Stage 05 — In Development | SDET / Developer testing | What code-level tests verify the changed code? | Executable **unit-test code** with real results |
| [`amrit-execute-qa-validation`](../skills/amrit-execute-qa-validation/README.md) | Stage 07 — In QA | QA Tester / QA Automation Engineer | Does the deployed build satisfy the agreed requirements and test cases? | QA **execution results and evidence** |

`amrit-test-jira-ticket` routes; it is not a pipeline that runs all three. It establishes the ticket's status, the artifacts that actually exist — requirements, approved design, QA test cases, implementation, unit tests, deployed build — and the user's intent, then selects only the activities the evidence supports and states every exclusion with its reason.
...
Acceptance Criterion
        ↓
QA Test Case                  amrit-draft-test-cases
        ↓
Implementation                amrit-implement-jira-ticket
        ↓
Unit Tests                    amrit-write-unit-tests
        ↓
QA Execution                  amrit-execute-qa-validation
        ↓
Evidence
        ↓
PASS / Defect
...
`amrit-create-brd` prepares **Draft — Pending Human Review**. Mandatory Confluence research is read-only by default, publishing requires an explicit request after the draft is shown, and the skill never claims that the exit criterion has been met.
...
`amrit-create-product-backlog` prepares **Draft - Pending Product Manager Review**. Jira remains read-only during drafting and refinement. Publication requires approval or finalization of the specific backlog and a separate explicit Jira creation request.
...
`amrit-create-technical-design` finishes with **Ready for Architect Review** and **No implementation should begin until the design is reviewed.** It is read-only, produces no implementation code, never modifies or publishes to Jira or Confluence, and never claims that the exit criterion has been met.

### QA test design at Stage 03

Stage 03 has two parallel responsibilities. Alongside the technical design, `amrit-draft-test-cases` answers the QA question: **what must QA test to prove that this requirement works?**

```text
Stage 03 — Analysis
    amrit-create-technical-design      Technical Architect
    amrit-draft-test-cases             QA Tester / Test Analyst
```
...
`amrit-draft-test-cases` finishes with **QA test specification drafted. Pending QA review.** or **QA test specification incomplete. Resolve the ambiguities above before QA review.**
...
A future `check-definition-of-ready` skill could verify linked artifacts and dependency readiness read-only, but none exists in this repository and none is implied by the Stage 05 skills. `amrit-implement-jira-ticket` assumes the ticket already reached Stage 05; it does not perform the Stage 04 readiness check and never claims a ticket is sprint-ready.

QA test cases are a Definition of Ready artifact. `amrit-test-jira-ticket` performs no activity at Stage 04, but when a ticket reaches it without agreed QA test cases, that absence is reported as a readiness gap and `amrit-draft-test-cases` is recommended.

## Stage 05 — In Development

`amrit-implement-jira-ticket` is the single entry point. It orchestrates the engineering personas the ticket actually requires and hands off to `amrit-create-development-pr`:
...
| --- | --- | --- |
| Technical Architect | `amrit-review-implementation-architecture` | cross-cutting or cross-repository change, new component or contract, changed ownership or integration boundary, security- or performance-material change, suspected design deviation |
| DBA / Database Engineer | `amrit-implement-database-change` | a schema object, migration, index, constraint, or data-compatibility concern exists |
| Backend Developer | `amrit-implement-backend-change` | services, APIs, controllers, domain logic, integrations, validation, persistence integration, error handling, or backend configuration change |
| Frontend Developer | `amrit-implement-frontend-change` | web UI, components, state, API integration, forms, client validation, accessibility, or frontend error handling change |
| Android / Kotlin Developer | `amrit-implement-android-change` | the Android applications change, including offline behaviour and platform constraints |
| UX / UI Specialist | `amrit-validate-ux-implementation` | a user-visible change exists and approved UX or design-system rules apply |
| SDET / developer testing | `amrit-write-unit-tests` | production behaviour changed — effectively always |

Representative routes:

```text
Backend-only        amrit-implement-backend-change → amrit-write-unit-tests → verification
Backend + database  amrit-implement-database-change → amrit-implement-backend-change → amrit-write-unit-tests
Full stack          amrit-implement-backend-change → amrit-implement-frontend-change
                    → amrit-validate-ux-implementation → amrit-write-unit-tests
Android             amrit-implement-android-change → amrit-write-unit-tests
Cross-cutting       amrit-review-implementation-architecture → amrit-implement-database-change
architectural       → amrit-implement-backend-change → amrit-implement-frontend-change
change              → amrit-validate-ux-implementation → amrit-write-unit-tests
```
...
- Persona classification and orchestration report — `amrit-implement-jira-ticket`
- Architecture conformance assessment — `amrit-review-implementation-architecture`
- `AMRIT-DB` migration and schema contract — `amrit-implement-database-change`
- Backend, frontend, and Android code changes — the implementation specialists
- UX conformance assessment — `amrit-validate-ux-implementation`
- Code-level unit tests with executed results — `amrit-write-unit-tests`
- Local verification results — `amrit-implement-jira-ticket` and `amrit-create-development-pr`
- Pull Request targeting the appropriate `release-X.Y.Z` branch — `amrit-create-development-pr`
- Architecture, DBA, UX, and code review sign-off — human review, never claimed by any skill

### Exit criterion

The Pull Request is approved with code-review sign-off, CI is green, and the change is squash-merged to the appropriate release branch. The ticket then moves to [Stage 06 — Pending QA](#stage-06--pending-qa), which deliberately carries no dedicated skill, and on to [Stage 07 — In QA](#stage-07--in-qa), where `amrit-execute-qa-validation` performs QA execution.

`amrit-implement-jira-ticket` finishes with either **Implementation complete and locally verified. Ready for PR preparation.** or **Implementation incomplete. Resolve the items above before PR preparation.**

`amrit-create-development-pr` finishes with either **Development PR created. Awaiting code review.** or **Development PR not created. Resolve the items above before retrying.**
...
The specialists own their layers. `amrit-implement-backend-change`, `amrit-implement-frontend-change`, `amrit-implement-android-change`, and `amrit-implement-database-change` edit code, each inspecting the code it owns before changing it. `amrit-write-unit-tests` writes and runs code-level unit tests — developer testing, kept separate from `amrit-draft-test-cases` at Stage 03 and `amrit-execute-qa-validation` at Stage 07. `amrit-review-implementation-architecture` and `amrit-validate-ux-implementation` are read-only assessments that report deviations and gaps for human decision. No specialist performs Git or Pull Request work, and none claims architecture, DBA, UX, code-review, QA, CI, or release approval; absent approvals are reported as absent.

`amrit-write-unit-tests` is also reachable through the testing meta-skill. `amrit-test-jira-ticket` routes to the same specialist when a ticket is in development and development-level testing is explicitly appropriate. The Stage 05 relationship above is unchanged, and no unit-testing logic is duplicated:

```text
amrit-implement-jira-ticket → amrit-write-unit-tests     (primary, Stage 05)
amrit-test-jira-ticket      → amrit-write-unit-tests     (development-level testing)
```

`amrit-create-development-pr` performs the Git and GitHub write operations — branch, commit, push, and Pull Request creation against a validated `release-X.Y.Z` branch — but performs no substantive implementation. Jira remains strictly read-only. It never approves, merges, or squash-merges a Pull Request, never claims code-review sign-off or green CI it did not observe, and stops and returns the work to implementation when the change is materially incomplete or a required `AMRIT-DB` change is missing.
...
Existing QA test cases may legitimately be reviewed or refined here through `amrit-draft-test-cases`, whose canonical creation point remains Stage 03. When it is re-run for a review, it states which test cases were added, changed, or retired, and never renumbers an existing one.

## Stage 07 — In QA

`amrit-execute-qa-validation` is the QA execution skill. It answers: **does the deployed implementation satisfy the previously agreed requirements and test cases?**
...
`amrit-execute-qa-validation` consumes the Stage 03 specification as written. It does not regenerate it from the implementation, and it never edits an expected result so the build passes. If the specification expects a duplicate-validation rejection and the build returns HTTP 500, the verdict is `FAIL`.
...
Failures are structured — preserved `TC` and `AC` identifiers, separated expected and actual behaviour, deterministic reproduction steps, build identity, evidence references, and reproducibility — so they serve as clean inputs to [`amrit-perform-root-cause-analysis`](../skills/amrit-perform-root-cause-analysis/README.md) when a failure requires root-cause investigation.

`amrit-execute-qa-validation` finishes with exactly one of **QA VALIDATION COMPLETE — all agreed test cases executed and passed. QA approval remains a human decision.**, **QA VALIDATION INCOMPLETE — <n> failed, <n> pending human or device execution.**, or **QA EXECUTION BLOCKED — <reason>. QA status: NOT EXECUTED.**
...
`amrit-execute-qa-validation` provides the evidence the human QA tester needs to make that decision — including everything that did not run — and never claims to be that approver, never records QA sign-off, and never transitions a ticket to QA Approved.
...
`amrit-test-jira-ticket` is the testing entry point across Stage 03, Stage 05, and Stage 07.
...
`amrit-prepare-release-notes` documents a release once its contents are complete and assigned a Jira Fix Version. It derives the release-note **format** from the latest applicable Confluence release notes and the release **content** from Jira.

```text
amrit-implement-jira-ticket → amrit-create-development-pr
        ↓
amrit-execute-qa-validation
        ↓
...
Unlike `amrit-perform-root-cause-analysis`, this skill requires no source-code inspection and does not use DeepWiki or Graphify in normal operation. Release membership comes from the Jira Fix Version and is never derived from Git commits, Pull Requests, branch contents, Confluence text, or issue similarity.
...
`amrit-prepare-release-notes` finishes with one of **Release Notes Status: Draft — Pending Human Confirmation**, **Release Notes Status: Confirmed — Published to Confluence**, **RELEASE NOTES BLOCKED — target release could not be resolved from Jira**, or **RELEASE NOTES BLOCKED — current Confluence template could not be inspected**.
...
`amrit-perform-root-cause-analysis` investigates production defects, support incidents, reopened defects, and QA failures to establish an evidence-backed root cause and propose corrective and preventive actions (CAPA).
...
production or support defect requires investigation
        ↓
amrit-perform-root-cause-analysis
        ↓
...
- QA failure evidence from `amrit-execute-qa-validation`
...
`amrit-perform-root-cause-analysis` uses an incident-first investigation order, intentionally different from the `amrit-answer-codebase-questions` research sequence:
...
`amrit-perform-root-cause-analysis` finishes with one of **RCA Status: Draft — Pending Human Confirmation**, **RCA Status: Confirmed — Published to Confluence**, or **RCA BLOCKED — relevant current source code could not be inspected**.
...
`amrit-answer-codebase-questions` researches DeepWiki first, Confluence when
