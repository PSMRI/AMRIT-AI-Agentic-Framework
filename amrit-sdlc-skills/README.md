...
Each skill is self-contained. Download either [`skills/create-brd`](skills/create-brd/README.md) or [`skills/create-product-backlog`](skills/create-product-backlog/README.md), retain its folder structure, and install only that folder through Claude Desktop's Add Skills flow. Neither skill depends on root-level files or the other skill at runtime.
